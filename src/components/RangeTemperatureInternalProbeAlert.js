...
const STORAGE_KEY = 'internalProbeTemperatureAlert';

export default function RangeTemperatureInternalProbeAlert({min, max}){
    const [minSelectedValue, setMinSelectedValue] = useState(20);
    const [maxSelectedValue, setMaxSelectedValue] = useState(30);
    ...
    const onChangeRangeTemperatureAlertValue = (value) => {
        return sendSMS(value ? createMessageEnableTemperatureAlert() : createMessageDisableTemperatureAlert()).then((response) => {
    ...
    const onValidateRangeTemperatureValue = () => {
        return sendSMS(createMessageTemperatureRangeAlert(minSelectedValue, maxSelectedValue)).then(() => {
